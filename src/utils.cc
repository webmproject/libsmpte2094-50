...
#if defined(__CRUBIT__) || defined(BAZEL_BUILD)
...
utils_rs::DynamicMetadata ToRustMetadata(const DynamicMetadata & cpp)
{
    ...
    rust.use_reference_white_tone_mapping_flag = cpp.use_reference_white_tone_mapping_flag;
    rust.hdr_reference_white = cpp.hdr_reference_white;
    rust.baseline_hdr_headroom_log2 = cpp.baseline_hdr_headroom_log2;
    for (int i = 0; i < 8; ++i) {
        rust.gain_application_space_chromaticities[i] = cpp.gain_application_space_chromaticities[i];
    }
...
void FromRustMetadata(const utils_rs::DynamicMetadata & rust, DynamicMetadata & cpp)
{
    cpp.has_adaptive_tone_map_flag = rust.has_adaptive_tone_map_flag;
    cpp.use_reference_white_tone_mapping_flag = rust.use_reference_white_tone_mapping_flag;
    cpp.hdr_reference_white = rust.hdr_reference_white;
    cpp.baseline_hdr_headroom_log2 = rust.baseline_hdr_headroom_log2;
    for (int i = 0; i < 8; ++i) {
        cpp.gain_application_space_chromaticities[i] = rust.gain_application_space_chromaticities[i];
    }
...
absl::StatusOr<std::string> ToSt209450(const DynamicMetadata & metadata)
{
    ...
    const ::utils_rs::ToSt209450Result result = utils_rs::to_st209450_ffi(rs_metadata);
    if (!result.success) {
...
absl::StatusOr<DynamicMetadata> FromSt209450(absl::string_view data)
{
    const ::utils_rs::FromSt209450Result result =
        utils_rs::from_st209450_ffi(absl::MakeConstSpan(reinterpret_cast<const uint8_t *>(data.data()), data.size()));
    if (!result.success) {
        return absl::InvalidArgumentError(result.get_error_message());
    }
...
{
    utils_rs::DynamicMetadata rs_metadata = ToRustMetadata(metadata);
    const auto result = utils_rs::dynamic_metadata_populate_pchip_slopes_ffi(rs_metadata);
    if (!result.success) {
        return absl::InvalidArgumentError(result.get_error_message());
    }
    FromRustMetadata(rs_metadata, metadata);
